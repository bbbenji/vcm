...
export const useMatStore = defineStore("mat", () => {
  ...
  function updateCell(row: number, col: number, isSecondary = false) {
    ...
    if (tool.type === "eraser") {
      ...
      // Enforce single-placement of Robot ('Bot') and Goal ('EvCharger') on the top grid
      if (!isSecondary && (tool.value === "Bot" || tool.value === "EvCharger")) {
        for (let r = 0; r < gridSize.value; r++) {
  ...
  function startSimulation() {
    ...
    if (!startChar) {
      ...
      const hasGoalJ10 = gridData.value[9]?.[9]?.icon === "EvCharger";
      const hasGoalJ1 = gridData.value[0]?.[9]?.icon === "EvCharger";
  ...
  function isSuccessCell(r: number, c: number): boolean {
    ...
    return cell.icon === "EvCharger" || cell.bg === "#eab308";
  }
