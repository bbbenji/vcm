...
const selectedToolClass = "!border-primary dark:!border-primary scale-110 shadow-[0_0_0_3px_rgba(99,102,241,0.35)] dark:shadow-[0_0_0_3px_rgba(99,102,241,0.5)]";
...
<template>
  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="flex items-baseline gap-1.5 min-w-0">
          <span class="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider select-none shrink-0 leading-none">
            {{ store.t.simulator }}
          </span>
          <span class="text-xs font-bold text-slate-700 dark:text-slate-200 truncate leading-none">
            <template v-if="store.simulationStatus === 'ready' && !store.isSimulating">{{ store.t.simReady }}</template>
            <template v-else-if="store.simulationStatus === 'paused'">{{ store.t.simPaused }}</template>
            <template v-else-if="store.simulationStatus === 'running'">{{ store.t.simStep }} {{ store.simulationStep }}/{{ store.simulationSteps.length }}</template>
            <template v-else-if="store.simulationStatus === 'success'">{{ store.t.simSuccess }}</template>
            <template v-else-if="store.simulationStatus === 'collision'">{{ store.t.simCollision }}</template>
            <template v-else-if="store.simulationStatus === 'out_of_bounds'">{{ store.t.simOutOfBounds }}</template>
            <template v-else-if="store.simulationStatus === 'ready' && store.isSimulating">{{ store.t.simEnd }}</template>
          </span>
        </div>
      </div>
      ...
      <div class="flex flex-row items-center justify-between gap-1.5">
        ...
        <div class="flex items-center gap-1">
          ...
          >
            ...
            <span>{{ store.simulationStatus === 'paused' ? store.t.simResume : store.t.simStart }}</span>
          </button>
        ...
        <div class="flex items-center gap-1 pl-2 border-l border-slate-200 dark:border-slate-800">
          <button 
            ...
            :class="store.simulationSpeed === 1500 ? 'bg-primary text-white font-bold' : 'bg-slate-100 dark:bg-slate-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800'"
          ...
          <button 
            ...
            class="h-7 w-7 rounded-md cursor-pointer flex items-center justify-center transition-colors"
            :class="store.simulationSpeed === 800 ? 'bg-primary text-white font-bold' : 'bg-slate-100 dark:bg-slate-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800'"
          ...
          <button 
            ...
            class="h-7 w-7 rounded-md cursor-pointer flex items-center justify-center transition-colors"
            :class="store.simulationSpeed === 300 ? 'bg-primary text-white font-bold' : 'bg-slate-100 dark:bg-slate-850 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800'"
    ...
    <div class="flex flex-row overflow-x-auto whitespace-nowrap p-2 gap-1 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/80 scrollbar-none shrink-0">
      <button
        ...
        :class="{ 'bg-white dark:bg-slate-800 text-primary dark:text-primary shadow-sm font-bold border border-slate-100 dark:border-slate-700/30': activeTab === cat.id }"
      ...
      >
        ...
        <span class="text-[0.62rem] md:text-[0.7rem] text-center whitespace-normal leading-tight break-words max-w-full font-medium">
    ...
    <div class="py-3 px-3 md:p-5 flex-1 overflow-y-auto bg-white dark:bg-slate-900 transition-colors duration-300">
      <div class="grid grid-cols-[repeat(auto-fill,minmax(2.5rem,1fr))] md:grid-cols-[repeat(auto-fill,minmax(3rem,1fr))] gap-2.5">
        ...
        >
          <component :is="getIcon('Eraser')" class="text-rose-500 dark:text-rose-400 w-5 h-5 md:w-6 md:h-6" />
        </button>
        ...
        <template v-for="cat in categories" :key="'grid-' + cat.id">
          <template v-if="activeTab === cat.id">
            <button
              ...
              :title="item"
            >
              <component v-if="cat.toolType === 'icon'" :is="getIcon(item)" class="w-5 h-5 md:w-6 md:h-6" />
          ...
          <div class="flex flex-col gap-3.5 col-span-full pb-6">
            ...
            >
              <div class="flex justify-between items-center w-full">
                <span class="font-bold text-slate-800 dark:text-slate-100 text-xs md:text-sm tracking-tight leading-tight">
                  {{ getTemplateName(tpl) }}
                </span>
                <span 
                  class="text-[8px] md:text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider select-none shrink-0"
                  :class="tpl.type === 'premade' ? 'bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300' : 'bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-300'"
                >
                  {{ tpl.type === 'premade' ? store.t.pattern : store.t.task }}
                </span>
              </div>
              <p class="text-[10px] md:text-[11px] text-slate-500 dark:text-slate-400 mt-1.5 leading-normal break-words">
                {{ getTemplateDesc(tpl) }}
              </p>
              <span class="text-[10px] text-primary font-bold mt-3 flex items-center gap-1.5 hover:translate-x-1 transition-transform">
