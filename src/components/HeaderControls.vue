...
<template>
  <header
    class="flex flex-col md:flex-row justify-between items-center p-4 md:px-8 bg-white shadow-sm sticky top-0 z-10 gap-4 md:gap-0"
  >
    <div class="flex items-center">
      <h1 class="text-2xl font-bold text-primary tracking-tight m-0">Wirtualna Mata</h1>
    </div>

    <div class="flex flex-wrap items-center justify-center gap-4">
      <div class="flex items-center gap-2 font-medium">
        <label for="grid-size">Rozmiar:</label>
        <select
          id="grid-size"
          :value="store.gridSize"
          @change="onSizeChange"
          class="px-4 py-2 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-800 outline-none focus:border-primary transition-colors cursor-pointer"
        >
      ...
      <button
        ...
        title="Cofnij (Undo)"
        class="flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all bg-slate-100 text-slate-700 hover:bg-slate-200 active:translate-y-0 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 cursor-pointer"
      >
        <Undo2 :size="18" /> Cofnij
      </button>

      <button
        @click="store.clearBoard"
        title="Wyczyść matę"
        class="flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all bg-red-100 text-red-500 hover:bg-red-300 hover:text-red-700 active:translate-y-0 hover:-translate-y-0.5 cursor-pointer"
      >
      ...
      <button
        ...
        class="flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all bg-indigo-50 text-indigo-600 hover:bg-indigo-100 active:translate-y-0 hover:-translate-y-0.5 cursor-pointer"
      >
      ...
      <div class="relative">
        <button
          ...
          title="Pobierz matę"
          class="flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all bg-primary text-white hover:bg-primary-hover hover:shadow-md hover:shadow-blue-500/25 active:translate-y-0 hover:-translate-y-0.5 cursor-pointer"
        >
          <Download :size="18" /> Pobierz <ChevronDown :size="16" />
        </button>
